...
$uploadDir = $php_path . 'storage/images/' . $ymd . '/';
...
//echo '/storage/images/' . $ymd . '/' . $fileName;
//exit;
// Return Success JSON-RPC response
//die('{"jsonrpc" : "2.0", "result" : null, img : '+$fileName+'}');

// 上传到7牛
$url = $_SERVER["DOCUMENT_ROOT"] . '/storage/images/' . $ymd . '/' . $fileName;
require $_SERVER["DOCUMENT_ROOT"] . '/../bootstrap/autoload.php';
list($ret, $err) = qiniu($url);
if ($err !== null) {
    return json_encode(['status' => 0, 'info' => $err]);
} else {
    unlink($url);
    echo env('QINIU_LINK') . $ret['key'];
    exit;
}
